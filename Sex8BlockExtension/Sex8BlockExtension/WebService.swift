    ...
    func read<P : Codable>(caller : WebserviceCaller<P>) throws {
        ...
        case .get:
            ...
            if let url = URL(string: urlString) {
                ...
                runningTask = session.dataTask(with: request, completionHandler: responseHandler)
                runningTask?.resume()
                return
            }
        ...
        case .post:
            ...
            do {
                let paraJSON = caller.paras.postParams().data(using: .utf8) //try JSONSerialization.data(withJSONObject: caller.paras, options: .prettyPrinted)
                if let url = URL(string: urlString) {
                    var request = URLRequest(url: url)
                    request.httpBody = paraJSON
                    request.httpMethod = caller.way.rawValue
                    runningTask = session.dataTask(with: request, completionHandler: responseHandler)
                    runningTask?.resume()
                    return
