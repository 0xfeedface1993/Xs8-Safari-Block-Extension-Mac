...
struct InfoRuleOption {
    ...
    /// 图片链接
    // ([^>]+class=\"zoom\"[^>]+)|(<img((\\ssrc=\"\\w+:[^\"]+\")|(\\salt=\"\\w+\\.\\w+\")|(\\stitle=\"\\w+\\.\\w+\")){3})
    static let imageLink = ParserTagRule(tag: "img", isTagPaser: true, attrubutes: [ParserAttrubuteRule(key: "file"), ParserAttrubuteRule(key: "href"), ParserAttrubuteRule(key: "src")], inTagRegexString: "([^>]+class=\"zoom\"[^>]+)|(((\\ssrc=\"\\w+:[^\"]+\")|(\\salt=\"\\w+\\.\\w+\")|(\\stitle=\"\\w+\\.\\w+\")){3})", hasSuffix: false, innerRegex: nil)
    /// 主内容标签
    ...
    private func fetchGroup(start: UInt, offset: UInt) {
        ...
        for x in 0..<splitGroupCount {
            ...
            for y in 0..<radio {
                ...
                let fetchURL = FetchURL(site: "xbluntan.net", board: .netDisk, page: startIndex + index, maker: maker)
        ...
        let linksRadio = 10
        let linksPageCount = linksCount / linksRadio + 1
        
        print("总共解析到 \(linksCount) 个内容链接")
        
        for x in 0..<linksPageCount {
            group = DispatchGroup()
            for y in 0..<linksRadio {
                let index = x * linksRadio + y
                if index >= linksCount {
                    break
                }
